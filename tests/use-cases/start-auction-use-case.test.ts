...
function makeScheduledAuction(lot: Lot, startAt: Date) {
  const now = new Date()
...
describe('StartAuctionUseCase', () => {
  beforeEach(() => {
    auctionRepository = new InMemoryAuctionRepository()
    userRepository = new InMemoryUserRepository()
    sut = new StartAuctionUseCase(auctionRepository, userRepository)
  })
  ...
  it('should be able to start a scheduled auction when startAt has passed', async () => {
    const lot = makeLot()
    const auction = makeScheduledAuction(lot, new Date(Date.now() - 60 * 1000))
    const user = makeUser()
