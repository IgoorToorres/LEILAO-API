...
function makeLot() {
  ...
  return Lot.create({
    ...
    quantity: 1,
    startingPrice: Money.create(1000), // R$ 10,00
    createdAt: now,
    updatedAt: now,
  })
}
...
function placeBid(auction: Auction, lot: Lot, userId: UniqueEntityId) {
  auction.placeBid(userId, lot.id, Money.create(1500))
}

describe('FinishAuctionUseCase', () => {
  beforeEach(() => {
    auctionRepository = new InMemoryAuctionRepository()
    userRepository = new InMemoryUserRepository()
    sut = new FinishAuctionUseCase(auctionRepository, userRepository)
  })
  ...
  it('should be able to finish a running auction with bids', async () => {
    const lot = makeLot()
    const auction = makeRunningAuction(lot)
    const user = makeUser()

    placeBid(auction, lot, user.id)
